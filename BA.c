...
#include<stdlib.h>
#include"BA.h"
int* BAnet(int max_vertex, int complit_graphV, int addn){

    double allD=0;
    double RM=RAND_MAX;
    int* BAnetwork=(int*)calloc(max_vertex*max_vertex,sizeof(int));
    int* D=(int*)calloc(max_vertex,sizeof(int));
    ...
    puts("B");
    srand(0);
    int* choosed_list=(int*)calloc(max_vertex,sizeof(int));
...
int* makeComplitGraph(int vn){
    int* network=(int*)calloc(vn*vn,sizeof(int));
    for(int i=0;i<vn;i++){
        for(int j=0;j<vn;j++){
        if(i!=j){
            network[i*vn+j]=1;
